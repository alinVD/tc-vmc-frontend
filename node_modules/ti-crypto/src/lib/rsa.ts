...
export interface OldExternalRSAKeyPair {
    ...
    key: {                                    // The key pair.
        ...
        privateKey: {                         // Wrapped private key info.
...
export function convertOldKeyPair(old: OldExternalRSAKeyPair, pwd: Uint8Array): Promise<ExternalRSAKeyPair> {
    ...
    return new Promise<ExternalRSAKeyPair>((resolve, reject) => {
        let oldWrapped = base64ToBytes(old.key.privateKey.key);
        let salt = base64ToBytes(old.key.privateKey.salt);
        let rounds = old.key.privateKey.iterations;
        let iv = base64ToBytes(old.key.privateKey.iv);
                    ...
                    aesKey => {
                        crypto.subtle.decrypt({
                            ...
                            oldUnwrapped => {
                                ...
                                let pkcs8Key = encodeToDER(SEQUENCE, concat(version, rsaAlgo, privateKey));
